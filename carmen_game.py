...
def processa_mandado():
    ...
    if len(suspeitos) == 1:
        ...
        print ("Não há suspeitos compatíveis com as características fornecidas. Volte aqui quando tiver mais informações.")
        mandado = None
        return mandado
...
def sorteia_caracteristica(culpado):
    ...
    if sorteada == 3:
        if culpado.genero == "Homem":
            return "Ouvi partes de sua conversa no celular e ele parecia gostar de ser "+ culpado.hobby + " nas horas vagas."
        else:
            return "Ouvi partes de sua conversa no celular e ela parecia gostar de ser "+ culpado.hobby + " nas horas vagas."
    if sorteada == 4:
        if culpado.genero == "Homem":
...
def investigar(culpado, prox_pais, caracteristicas):
    ...
    os.system('clear')
    if escolha == "a":
        print("Gerente: A pessoa que você procura esteve aqui e trocou seu dinheiro por", prox_pais.moeda)
        print (caracteristicas['banco'])   
    elif escolha == "b":
        print("Comissário de Bordo: A pessoa que você procura esteve aqui e partiu em um avião com uma bandeira", prox_pais.bandeira,"na asa.")
        print (caracteristicas['aeroporto'])
    elif escolha == "c":
        print("Bibliotecária: A pessoa que você procura esteve aqui e procurava informações sobre", prox_pais.cultura)
...
def principal():
    ...
    while True:
        ...
        if data_atual >= data_fim:
            if mandado is None:
                print ("Você não expediu um mandado para", culpado.nome,". Mais sorte da próxima vez!")
                print ("\n")
                print ("Pronto(a) para o próximo caso?")
                resposta = input ("Sim / Não: ")
                le_escolha_ir_prox_caso(resposta, dados.OPCOES)
                ...
                print ("Você expediu um mandado para a pessoa errada!", culpado,"escapou. Mais sorte da próxima vez!")
                print ("\n")
                print ("Pronto(a) para o próximo caso?")
                resposta = input ("Sim / Não:")
                le_escolha_ir_prox_caso(resposta, dados.OPCOES)
            elif culpado in mandado:
                print("Você prendeu", culpado.nome,".", culpado.nome, "estava com o tesouro nacional que será devolvido a seus habitantes. Parabéns, Detetive!")
